...
% vehicular: 
vtv= interp1([0:T_SS:T_tot], 1*randn(size([0:T_SS:T_tot])), t_tot_vec, 'spline');   
theta_tv= 0; 
vrv= 0;  
theta_rv= 0; 
...
fprintf(f1, '%f surface_height_(depth)_[m]\n', h0); 
fprintf(f1, '%f TX_height_[m]\n', ht0); 
fprintf(f1, '%f RX_height_[m]\n', hr0); 
fprintf(f1, '%f channel_distance_[m]\n', d0); 

fprintf(f1, '%f spreading_factor\n', k); 
fprintf(f1, '%f speed_of_sound_in_water_[m/s]\n', c); 
fprintf(f1, '%f speed_of_sound_in_bottom_[m/s]\n', c2); 
fprintf(f1, '%f minimum_relative_path_strength\n', cut); 

fprintf(f1, '%f minimum_frequency_[Hz]\n', fmin); 
fprintf(f1, '%f bandwidth_[Hz]\n', B); 
fprintf(f1, '%f frequency_resolution_[Hz]\n', df); 
fprintf(f1, '%f time_resolution_[seconds]\n', dt); 
fprintf(f1, '%f coherence_time_of_the_small-scale_variations_[seconds]\n', T_SS); 

fprintf(f1, '%f variance_of_S_S_surface_variations_[m^2]\n', sig2s); 
fprintf(f1, '%f variance_of_S_S_bottom_variations_[m^2]\n', sig2b); 
fprintf(f1, '%f 3-dB_width_of_the_p.s.d._of_intra-path_delays_[Hz]\n', B_delp); 
fprintf(f1, '%d number_of_intra-paths\n', Sp); 
fprintf(f1, '%f mean_of_intra-path_amplitudes\n', mu_p); 
fprintf(f1, '%f variance_of_intra-path_amplitudes\n', nu_p); 

fprintf(f1, '%f total_duration_of_the_simulated_signal_[seconds]\n', T_tot); 
fprintf(f1, '%f min_of_surface_height\n', h_bnd(1));
fprintf(f1, '%f max_of_surface_height\n', h_bnd(2));
fprintf(f1, '%f min_of_transmitter_height\n', ht_bnd(1));
fprintf(f1, '%f max_of_transmitter_height\n', ht_bnd(2));
fprintf(f1, '%f min_of_receiver_height\n', hr_bnd(1));
fprintf(f1, '%f max_of_receiver_height\n', hr_bnd(2));
fprintf(f1, '%f min_of_channel_distance\n', d_bnd(1));
fprintf(f1, '%f max_of_channel_distance\n', d_bnd(2));

fprintf(f1, '%f L_S_standard_deviation_of_surface_height_[m]\n', sig_h); 
fprintf(f1, '%f L_S_standard_deviation_of_transmitter_height_[m]\n', sig_ht); 
fprintf(f1, '%f L_S_standard_deviation_of_receiver_height_[m]\n', sig_hr); 
fprintf(f1, '%d L_S_standard_deviation_of_receiver_height_[m]\n', sig_d); 
fprintf(f1, '%f AR_parameter_for_generating_L_S_variations\n', a_AR); 
